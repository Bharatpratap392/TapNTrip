...
const TravelGuideDashboard = () => {
  ...
  const [services, setServices] = useState([]); // This will store fetched tours
    ...
    const fetchData = async () => {
      if (!authLoading && user) {
        try {
          ...
          // Fetch Services (Tours)
          const fetchedTours = await guideServicesDB.getTours();
          setServices(fetchedTours);
  ...
  return (
    ...
    >
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-8">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-6">Tour Guide Dashboard</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <motion.div variants={cardVariants} whileHover="hover" className="bg-green-50 dark:bg-green-900/50 p-6 rounded-lg shadow">
            <h2 className="text-xl font-semibold text-green-800 dark:text-green-200 mb-2">Active Tours</h2>
